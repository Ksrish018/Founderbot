...
def parse_feed(xml: str | bytes, query: str) -> list[NewsItem]:
    ...
    for e in feed.entries:
        ...
        src = getattr(getattr(e, "source", None), "title", None) or (e.get("source") or {}).get("title", "")
        ...
        items.append(NewsItem(query, title, src or "", e.get("link", ""), pub, snippet[:400]))
    return items


def filter_items(items: list[NewsItem], lookback_days: int, now: datetime | None = None) -> list[NewsItem]:
...
async def fetch(queries: list[str], lookback_days: int, timeout: float = 15.0) -> list[NewsItem]:
    ...
    return filter_items([i for r in ok for i in r], lookback_days)
...
async def find_angle(db: DB, gemini: Gemini, settings: Settings, request_id: int, note_text: str) -> None:
    ...
    try:
        ...
        queries = [s.strip() for s in q.data.queries if s.strip()][:3]
        items = await fetch(queries, settings.news_lookback_days)
    ...
    if not items:
        db.run("UPDATE draft_requests SET news_status='none', news_reason=?, news_ranking='[]', news_pos=-1 WHERE id=?",
               (f"No news in the last {settings.news_lookback_days} days for: {', '.join(queries)}.", request_id))
        return

    ap, _ = load_prompt("angle")
    listing = "\n".join(f"[{i}] {it.title} | {it.source} | {it.published:%d %b %Y} | {it.snippet}"
...
def item_block(item) -> str:
    ...
    return (f"NEWS ITEM (reference only what the title and snippet state; attribute to the source):\n"
